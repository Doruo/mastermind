...
public class Main { // à renommer "MasterMindBase" lors du rendu
    ...
    */
      public static int[] propositionCodeHumain(int nbCoups, int lgCode, char[] tabCouleurs)
    {
        ...
        int[] propCodeHumain;
        propCodeHumain = new int [lgCode.length];
        
        System.out.println("Saisir proposition n°"+nbCoups+" de couleurs");
        
        
        for (int i=0; i<lgCode.length;i++)
        {   
            while( )
            saisie = scanner.nextInt();
    ...
    // Initialisation saisie début de partie
    public static void initMasterMind ()
    {
        System.out.println("Bienvenue dans le MasterMind"); sautLigne();
